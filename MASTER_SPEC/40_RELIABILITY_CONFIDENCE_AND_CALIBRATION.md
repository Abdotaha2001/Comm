...
- **H.6 Independence caveat.** Multiplicative composition (§H.1 product) assumes the stages fail **independently**. For **positively correlated** inputs — e.g. the two endpoint detections of one inter-frame displacement (same detector, adjacent frames, near-identical conditions) — the product **under**-estimates the joint confidence (it double-counts shared error) and is only a conservative lower bound. Where the components are both **required** and strongly correlated, the implementation **MUST** use the **minimum** (§H.4 `required_all`), which is exact under perfect correlation and avoids the spurious independence penalty; the `chain` product is reserved for genuinely independent pipeline stages. The chosen composition mode **MUST** be recorded in `source` (e.g. `composition: required_all`).
...
|----------|------|------|--------|
| Reliability envelope + cap (capture) | `backend/app/capture_quality.py` (`build_reliability_envelope`, `reliability_ceiling_for`) | capture envelope + tier cap | ✅ |
...
| OOD / novelty gate | `backend/app/ood.py` (+ `benchmark.detection_rate_samples`) | standardized-distance novelty gate (§Y); detection-rate reference **fit from the golden set** (std-floored) — not hard-coded | ✅ |
| Per-domain abstain thresholds | `reliability.DOMAIN_THRESHOLDS` + `threshold_for` | config thresholds, no magic numbers (§K) | ✅ |
| Envelopes on pipeline outputs | `worker.py` (shots: speed+spin · events · run · scoreboard · OOD) + `analytics/profile.py` (profile) | every measured value carries an envelope (§C); speed composes its two correlated endpoint detections by **min** (`required_all`, §H.6), abstains on low measurement SNR (§BH); scoreboard consensus thresholded (§K); detection-rate OOD signal (§Y) | ✅ |
| Measurement uncertainty (GUM) | `backend/app/uncertainty.py` | first-order propagated speed interval — replaces the fabricated ±30% (§AP/§BH/§AM); **SNR significance gate** abstains where the linearisation is invalid / `‖Δp‖` is Rician-biased (§BH) | ✅ |
...
- **Validity regime (SNR significance gate).** First-order (GUM) propagation is a **linearisation** valid only at **high signal-to-noise**. For a speed derived from an inter-frame displacement, define `SNR = ||Δp|| / (√2·σ_px)` (per-endpoint localisation std σ_px ⇒ displacement-noise std √2·σ_px; note `SNR = 1/rel_disp`). Below a minimum (`SNR_MIN`, default **3**) the system **MUST abstain** rather than report a value, because (a) the linearisation is invalid (the relative localisation term → O(1)) and (b) the magnitude `||Δp|| = √(Δx²+Δy²)` is **Rician-biased high** — `E[||Δp||²] = ||Δp||²_true + 4σ_px²` — so the speed is systematically over-estimated at low SNR. The abstain decision **MUST** be tied to the **measurement's own SNR**, not merely to detection confidence. In the reported regime (SNR ≥ SNR_MIN) the residual magnitude bias (≈+10% at the boundary, falling as `2/SNR²`) **MUST** stay within the reported `±k·σ` interval. Implemented in `backend/app/uncertainty.py` (`speed_uncertainty` → `components.significant`) + enforced at emission in `worker.py` (`_speed_envelope`).
