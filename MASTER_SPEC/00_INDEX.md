...
| 41 | `41_SECURITY_PRIVACY_AND_COMPLIANCE.md` | 🔧 Authoritative security/privacy/compliance law (5th build pillar — trust & safety): data classification · AuthN/AuthZ (RBAC+ABAC+field-level) · tenant isolation · crypto/KMS · consent & DSR · tamper-evident audit · officiating signing · STRIDE+ML threat model · LLM/SSRF/media hardening · GDPR/biometric/EU-AI-Act mapping · security SLOs · CI-enforced security invariants (formalizes Part 31; honest status corrects Part 31 over-claims) |
| 42 | `42_OBSERVABILITY_SRE_AND_OPERATIONS.md` | 🔧 Authoritative operability law (6th build pillar — operability): three pillars (logs/metrics/traces) · SLI/SLO/error-budget · golden signals · health/readiness · **reliability & honesty SLIs** (abstention/calibration-drift/OOD) · alerting/on-call/runbooks · incident mgmt · capacity/perf · cost/FinOps · release & model-rollout observability · telemetry privacy · CI-enforced observability invariants (formalizes Part 34 §L/§AE + Part 13) |
| 43 | `43_DATA_AND_ML_LIFECYCLE_MLOPS.md` | 🔧 Authoritative intelligence law (7th build pillar — the model/data lifecycle): data sourcing/consent · annotation+IAA (Part 30) · dataset versioning/splits/no-leakage · feature/embedding store · reproducible training · the **multi-gate promotion** (accuracy+calibration+fairness+robustness+security+card) · model registry/cards/signing · progressive rollout · drift→retrain closed loop · multi-model cascade eval · eval rigor (CIs/significance) · generative/privacy-preserving/MRM · heuristic→model honesty · CI-enforced lifecycle invariants (formalizes Part 12; golden-set gate ✅) |
| 44 | `44_DELIVERY_RELEASE_ENGINEERING_AND_IAC.md` | 🔧 Authoritative delivery law (8th build pillar — shipping): CI/CD pipeline · trunk-based + protected main · reproducible/signed build & packaging · **infrastructure-as-code** + drift detection · environments & artifact promotion (build-once) · config/secrets delivery (12-factor) · **zero-downtime schema migrations** (expand→contract) · progressive delivery (canary/blue-green/flags) · release/versioning · rollback + DR/BCP · deploy governance + provenance · DORA · CI-enforced delivery invariants (formalizes Part 34/13; CI ✅, CD/IaC ⬜) |
| 45 | `45_FRONTEND_AND_CLIENT_ARCHITECTURE.md` | 🔧 Authoritative experience law (9th build pillar — the human-facing client): surfaces (web/mobile/desktop/courtside/overlay) · view-not-truth (server is authority) · **reliability-honest UI** (no bare numbers · abstain = "I don't know" · tier badges · uncertainty viz) · design system · **a11y (WCAG) + bilingual EN↔AR/RTL** · performance budgets · offline/PWA · real-time (WS/SSE) · honest data-viz · capture UX · client security · RUM · client testing · CI-enforced client invariants (formalizes Part 32/14; client greenfield, contracts ✅) |
